...
export function TaskFilterBar({ filters, members }: TaskFilterBarProps) {
    ...
    const hasActiveFilters =
        filters.search || filters.onlyMine || filters.teamId || filters.priority || filters.labelId || filters.componentId;
    ...
    return (
        <div className="flex flex-wrap items-center gap-2 surface border rounded-lg p-2">
            ...
            <select value={filters.teamId} onChange={(e) => filters.setTeamId(e.target.value)} className="input-base border rounded px-3 py-1.5 text-sm">
                <option value="">Tüm Takımlar</option>
                {uniqueTeams.map(([id, name]) => (
                    <option key={id} value={id}>{name}</option>
                ))}
            </select>
            <select value={filters.priority} onChange={(e) => filters.setPriority(e.target.value)} className="input-base border rounded px-3 py-1.5 text-sm">
                <option value="">Tüm Öncelikler</option>
                {Object.entries(PRIORITY_LABELS).map(([value, label]) => (
                    <option key={value} value={value}>{label}</option>
                ))}
            </select>
            <select value={filters.labelId} onChange={(e) => filters.setLabelId(e.target.value)} className="input-base border rounded px-3 py-1.5 text-sm">
            ...
            {components && components.length > 0 && (
                <select value={filters.componentId} onChange={(e) => filters.setComponentId(e.target.value)} className="input-base border rounded px-3 py-1.5 text-sm">
            ...
            {hasActiveFilters && (
                <button onClick={filters.reset} className="text-xs text-muted hover:text-red-500 whitespace-nowrap">
                    Filtreleri Temizle
                </button>
            )}
        </div>
