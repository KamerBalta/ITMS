...
export function ComponentsManagementPage() {
    ...
    const handleCreate = async () => {
        ...
        setError(null);
        try {
            await createComponent.mutateAsync({ name, description: description || undefined, leadUserId: leadUserId || null });
            setName('');
            setDescription('');
            setLeadUserId('');
        } catch (err) {
                ...
                <div className="surface border rounded-lg divide-y divide-gray-200 dark:divide-gray-700">
                    {components?.map((c) => (
                        <div key={c.id} className="p-3 flex items-center justify-between text-sm">
                            ...
                            <button onClick={() => handleDelete(c.id)} className="text-xs text-red-500 dark:text-red-400 hover:underline cursor-pointer">
                                Sil
                            </button>
                        </div>
                    ))}
                    {components?.length === 0 && <p className="text-sm text-muted p-3">Henüz component yok.</p>}
                </div>
            )}
