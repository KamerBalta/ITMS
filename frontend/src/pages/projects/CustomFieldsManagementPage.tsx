...
const TYPE_LABELS: Record<string, string> = { text: 'Metin', number: 'Sayı', select: 'Seçim Listesi', user: 'Kullanıcı' };
...
export function CustomFieldsManagementPage() {
    ...
    const handleCreate = async () => {
        ...
        setError(null);
        try {
            await createField.mutateAsync({ name: name.trim(), fieldType, isRequired });
            setName('');
            setIsRequired(false);
        } catch (err) {
                ...
                <div className="surface overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="divide-y divide-gray-200 dark:divide-gray-700">
                        {fields?.map((f) => (
                            <div
                                key={f.id}
                                className="group flex items-center gap-4 px-5 py-4 transition hover:bg-gray-50 dark:hover:bg-gray-900/50"
                            >
