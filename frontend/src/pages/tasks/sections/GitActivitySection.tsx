...
export function GitActivitySection({ taskId, issueKey }: { taskId: string; issueKey: string }) {
    ...
    const branchName = `${issueKey.toLowerCase()}-` + 'yeni-ozellik'; // #Git: onerilen branch adi -- kullanici sonuna kendi acikliyor ekleyebilir
    ...
    return (
        <div className="surface border rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
                <h2 className="font-semibold text-primary">Git Activity ({commits.length})</h2>
                <button onClick={copyBranchCommand} className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">📋 Branch komutu</button>
            </div>
                    ...
                    <li key={c.commitHash} className="text-sm border-b border-gray-100 dark:border-gray-800 last:border-0 pb-2 last:pb-0">
                        <div className="flex items-center justify-between">
                            {c.commitUrl ? (
                                ...
                                <span className="font-mono text-xs text-muted">{c.commitHash.slice(0, 7)}</span>
                            )}
                        ...
                        <p className="text-xs text-muted">{c.authorName}{c.branchName && ` · ${c.branchName}`}</p>
                    </li>
                ))}
            </ul>
        </div>
